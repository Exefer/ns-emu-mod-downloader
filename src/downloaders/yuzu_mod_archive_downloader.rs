...
pub struct YuzuModArchiveDownloader {
    client: Easy,
}
...
impl super::mod_downloader::ModDownloader for YuzuModArchiveDownloader {
    ...
    fn read_game_titles(
        &mut self,
    ) -> Result<Vec<crate::entities::game::Game>, Box<dyn std::error::Error>> {
        let dataset: ModDownloadDataset = serde_xml_rs::from_str(YUZU_MOD_ARCHIVE_FILE)?;
        ...
        let games: Vec<Game> = dataset
            ...
            .filter_map(|entry| {
                ...
                let mod_download_urls = self
                    .get_mod_download_urls(&repo_tree.tree, &entry.mod_url_path)
                    .ok()?;
